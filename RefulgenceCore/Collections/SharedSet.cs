...
// Mostly from https://raw.githubusercontent.com/Ottermandias/Penumbra.GameData/refs/heads/main/Structs/SharedSet.cs.
public struct SharedSet<TItem, TBitVector>(SharedSet<TItem, TBitVector>.Universe? possibleValues)
    : IEnumerable<TItem>, IEquatable<SharedSet<TItem, TBitVector>>, ICloneable
    where TItem : IEquatable<TItem> where TBitVector : unmanaged, IBinaryInteger<TBitVector>
{
    ...
    public static SharedSet<TItem, TBitVector> operator ~(SharedSet<TItem, TBitVector> value)
        => new(
            ...
            value.PossibleValues is null
                ? TBitVector.Zero
                : ((TBitVector.One << value.PossibleValues.Count) - TBitVector.One) ^ value._bitVector
        );
